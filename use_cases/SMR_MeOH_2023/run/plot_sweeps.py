...
def plot_hist(sweep_df):
  ...
  fig.tight_layout()
  fig.savefig(os.path.join(os.path.dirname(os.path.abspath(__file__)), "smr_meoh_results.png"))
  return None
...
def get_results():
  ...
  sweep_df = pd.DataFrame(columns=['location', 'name', 'mean_NPV', 'std_NPV', 'baseline_NPV', 'std_baseline_NPV', \
    'smr_capacity','htse_capacity','meoh_capacity', 'h2_storage_capacity'])
...
def main():
  ...
  os.chdir(dir)
  df = get_results()
  df.to_csv(os.path.join(dir,'smr_meoh_results.csv'))
  plot_hist(df)
